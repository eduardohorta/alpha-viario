...
class TestAssociados(unittest.TestCase):
    ...
    # P9 foi redefinido em 13/08/2026 como o entroncamento Rua Santuário × Av. Oscar Pereira e
    # georreferenciado por pin em 14/08/2026 (17 sinistros no limiar principal). Os totais abaixo
    # já o incluem; o antigo P9 (rótula Cristiano Kraemer × Juca Batista) foi retirado.
    def test_overlap_todas(self):
        self.assertEqual(self._overlap(self.rows), (883, 825, 58))

    def test_overlap_principais(self):
        princ = [r for r in self.rows if r["associacao_principal"] == "sim"]
        self.assertEqual(self._overlap(princ), (690, 664, 26))
